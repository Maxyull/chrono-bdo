...
#   - créer l'enregistrement DNS `chrono.maxyull.fr`, qui vit chez le
...
CONF_CHRONO="$SECRETS/chrono-bdo.env"
if [[ ! -f "$CONF_CHRONO" ]]; then
  ...
  cat >"$CONF_CHRONO" <<EOF
# Serveur de classement de Chrono. Généré le $(date +%F).
# Hors de tout dépôt git, comme tous les secrets.
CHRONO_PG_USER=chrono
CHRONO_PG_PASSWORD=$MDP
CHRONO_PG_DATABASE=chrono
CHRONO_PORT=8010
CHRONO_DOMAINE=chrono.maxyull.fr
EOF
fi
# shellcheck disable=SC1090
set -a; source "$CONF_CHRONO"; set +a

DEPOT="https://github.com/Maxyull/chrono-bdo.git"
CIBLE="/opt/chrono"

echo "==> déploiement sur ${VPS_HOST}, port ${CHRONO_PORT}, domaine ${CHRONO_DOMAINE}"

ssh -i "$VPS_SSH_KEY" -o StrictHostKeyChecking=no "${VPS_USER}@${VPS_HOST}" \
  CHRONO_PG_USER="$CHRONO_PG_USER" \
  CHRONO_PG_PASSWORD="$CHRONO_PG_PASSWORD" \
  CHRONO_PG_DATABASE="$CHRONO_PG_DATABASE" \
  CHRONO_PORT="$CHRONO_PORT" \
  CHRONO_DOMAINE="$CHRONO_DOMAINE" \
...
sudo -u postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='${CHRONO_PG_USER}'" | grep -q 1 || \
  sudo -u postgres psql -c "CREATE ROLE ${CHRONO_PG_USER} LOGIN PASSWORD '${CHRONO_PG_PASSWORD}';"
sudo -u postgres psql -tAc "SELECT 1 FROM pg_database WHERE datname='${CHRONO_PG_DATABASE}'" | grep -q 1 || \
  sudo -u postgres psql -c "CREATE DATABASE ${CHRONO_PG_DATABASE} OWNER ${CHRONO_PG_USER};"
...
# redémarrage suivant.
sudo -u postgres psql -c "ALTER ROLE ${CHRONO_PG_USER} PASSWORD '${CHRONO_PG_PASSWORD}';" >/dev/null
...
echo "--- reverse proxy"
sudo tee /etc/caddy/chrono.caddyfile >/dev/null <<CADDY
# Chrono BDO — classement des temps de quêtes.
...
${CHRONO_DOMAINE} {
	...
	reverse_proxy 127.0.0.1:${CHRONO_PORT}
}
CADDY
grep -q "chrono.caddyfile" /etc/caddy/Caddyfile || \
  echo "import /etc/caddy/chrono.caddyfile" | sudo tee -a /etc/caddy/Caddyfile >/dev/null
...
curl -fsS "http://127.0.0.1:${CHRONO_PORT}/sante" && echo
DISTANT
